...
class Remote(object):
    ...
    def __init__(self, uri=['http://127.0.0.1:8270'], timeout=None):
        ...
        for i in range(len(uri)):
            if '://' not in uri[i]:
                uri[i] = 'http://' + uri[i]
        with open('d:/mytest.txt', "a+") as tmp:
            tmp.write(json.dumps(uri))
        if timeout:
            timeout = timestr_to_secs(timeout)
        self._uris = uri
        self._clients = []
        for item in uri:
            self._clients.append(XmlRpcRemoteClient(item, timeout))

    def get_keyword_names(self, attempts=2):
        for i in range(attempts):
            time.sleep(i)
            try:
                return self._clients[0].get_keyword_names()
            except TypeError as err:
                error = err
        raise RuntimeError('Connecting remote server at %s failed: %s'
                           % (str(self._uris), error))

    def get_keyword_arguments(self, name):
        try:
            return self._clients[0].get_keyword_arguments(name)
        except TypeError:
            return ['*args']

    def get_keyword_tags(self, name):
        try:
            return self._clients[0].get_keyword_tags(name)
        except TypeError:
            return None

    def get_keyword_documentation(self, name):
        try:
            return self._clients[0].get_keyword_documentation(name)
        except TypeError:
            return None
    ...
    def run_keyword(self, name, args, kwargs):
        ...
        for client in self._clients:
            result = RemoteResult(client.run_keyword(name, args, kwargs))
            results.append(result.return_)
